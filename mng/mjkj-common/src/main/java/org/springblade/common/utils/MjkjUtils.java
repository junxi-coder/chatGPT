...
//		ImmutableMap<Long, Map<String, Object>> nodeMap = Maps.uniqueIndex(items, m -> getMap2Long(m, "id"));
		ImmutableMap<Long, Map<String, Object>> nodeMap = new ImmutableMap.Builder<Long, Map<String, Object>>()
			.putAll(items.stream()
				.collect(Collectors.toMap(m -> getMap2Long((Map<String, Object>) m, "id"), Function.identity()))
			)
			.build();
